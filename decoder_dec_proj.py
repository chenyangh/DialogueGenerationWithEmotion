...
class BeamSearchDecoder(object):
    ...
    def decode_batch(self, input_lines_src, _emo):
        ...
        for i in range(self.pad_len):
            ...
            trans_vocab = torch.bmm(dec_out.unsqueeze(1), self.model.decoder2vocab(tag).view(-1, self.model.trg_hidden_dim, self.model.vocab_size))

            out = F.softmax(trans_vocab).unsqueeze(0)
            word_lk = out.view(
                beam_size,
                remaining_sents,
                -1
    ...
    def translate(self, _emo):
        ...
        df.to_csv('outputs/decoder_vocablayer_beam' + str(self.beam_size) + '_' + str(_emo) + '.csv', encoding='utf-8', index=False)
