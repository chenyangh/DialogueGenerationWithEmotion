...
class BeamSearchDecoder(object):
    ...
    def decode_batch(self, input_lines_src, _emo):
        ...
        for i in range(self.pad_len):
            ...
            tag = torch.LongTensor([_emo] * trg_emb.size(0))
            tag = Variable(tag.view(-1)).cuda()
            ...
            dec_out_transformed = torch.bmm(dec_out.unsqueeze(1),
                self.model.emo_transform(tag).view(-1, self.model.trg_hidden_dim, self.model.trg_hidden_dim)).squeeze(1)
            # out = F.softmax(self.model.decoder2vocab(dec_out)).unsqueeze(0)
            out = F.softmax(self.model.decoder2vocab(dec_out_transformed)).unsqueeze(0)
            word_lk = out.view(
                beam_size,
                remaining_sents,
                -1
    ...
    def translate(self, _emo):
        ...
        df.to_csv('outputs/decoder_transformation_beam' + str(self.beam_size) + '_' + str(_emo) + '.csv', encoding='utf-8', index=False)
...
def main(beam_size):
    ...
    model_path = 'checkpoint_bak/new_transform_epoch_7.model'
