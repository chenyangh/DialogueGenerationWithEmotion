...
class BeamSearchDecoder(object):
    ...
    def decode_batch(self, input_lines_src):
        """Decode a minibatch."""
    ...
    def translate(self, _emo):
        ...
        for i, (src, trg) in tqdm(enumerate(self.data_loader),
            ...
            hypotheses, scores = self.decode_batch(Variable(src, volatile=True).cuda())
        ...
        df.to_csv('outputs/simple_beam' + str(self.beam_size) + '_foo_' + str(_emo) + '.csv', encoding='utf-8', index=False)
...
def main(beam_size):
    ...
    model_path = 'checkpoint/new_simple_foo_epoch_23.model'
    ...
    for __emo in range(9):
        test_set = EmotionDataLoaderFoo(X, y, __emo, pad_len, word2id)
